...
export default defineConfig({
  ...
  themeConfig: {
    ...
    footer: {
      ...
      copyright:
        '<a class="footer-cc-link" target="_blank" href="https://creativecommons.org/licenses/by-nc-sa/4.0/">CC BY-NC-SA 4.0</a> © 2024-PRESENT HackathonWeekly',
    },
    search: {
      provider: 'local',
      options: {
    ...
    sidebar: generateSidebar([
      ...
      // },
      {
        ...
        // includeRootIndexFile: true, // 包含根目录下的 index 文件, 默认显示为 index, todo 但会导致 nextPage 出错... 无限循环在当前页面
        useTitleFromFrontmatter: true, // 这样可以使用 frontmatter 中的 title 字段作为标题(而不是 index)
